...
def main(data):
    ...
    with tab1:
        ...
        if REMOVE_OUTLIERS:
            # Outlier Detection with Isolation Forest (via pyod)
            st.markdown("### Outlier Detection with Isolation Forest")

            # Select the contamination level (percentage of data considered outliers)
            contamination = st.slider("**Contamination (Outlier Proportion)**", 0.01, 0.2, 0.05)
    
            # Import Isolation Forest from pyod
            from pyod.models.iforest import IForest

            # Initialize the Isolation Forest model
            model = IForest(contamination=contamination)
    
            # Fit the model on the 'y' column
            model.fit(data[['y']])
    
            # Predict outliers: 1 = Outlier, 0 = Inlier
            data['outlier'] = model.labels_

            # Separate outliers and inliers
            outliers = data[data['outlier'] == 1]
            inliers = data[data['outlier'] == 0]

            # Replace outliers with None (to keep structure for imputation if needed)
            data.loc[data['outlier'] == 1, 'y'] = None

            # Drop the 'outlier' column
            data = data.drop(columns=['outlier'])

            st.markdown("#### Data After Outlier Removal")
            plot_data(data, original_columns)
...
if __name__ == '__main__':
    ...
    st.sidebar.info("""Made by Onkar Ithape & Aditi Paldhare""")
